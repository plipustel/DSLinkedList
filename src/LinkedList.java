...
class Record {
	...
	/*
	 * In Java, when you declare a variable like public String next;, it explicitly
	 * states that next is a reference to a String. You cannot later assign it a
	 * reference to an object of a different type (like a Record). This is because
	 * Java is statically typed, and the type of a variable is determined at
	 * compile-time.
	 * 
	 * When you declare a field like public Record next; in a class, you are saying
	 * that the variable next is a reference to an instance of the Record class. In
	 * other words, next will hold an object of type Record.
	 * 
	 */
...
class linkList {
	...
	/*
	 * Find a specific Link Find a 'record/link' based on a specific key. In this
	 * case we assume 'key' is the 'StdNo' Since we want 'find' method return an
	 * object that has a references or something like record/link that has a
	 * reference, so we declare it as a 'Link' datatype.
	 **/
	...
	public Record delete(int key) {
		...
		if (current == first) {
			...
			/*
			 * Terminate the node to be deleted, by keeping the previoius pointer to next
			 * node of current In other words, we connect the reference next link of
			 * previous to the next of current link (null).
			 *
			 * If we write something like this current = current.next : means we didn't
			 * connect anything to the previous link with the consequence the list looks
			 * like "drop".
			 **/
